...
AGENT_RENDER_EFFECTIVE_AUTO_MODE = AGENT_RENDER_MODE_MIRROR_SCREEN
AGENT_RENDER_MODES = {
    ...
    def create(self, session_token, terminal_id, sid, browser_id, browser_key, challenge, algorithm):
        ...
        with self._lock:
            ...
            for field in ('attempt_id', 'node_id'):
                if browser_key.get(field):
                    payload[field] = browser_key[field]
        ...
        with self._lock:
            ...
            for field_name in ('terminal_id', owner_field, 'key_id', 'challenge_sha256'):
                ...
                if (
                    ...
                    return 'ssh_browser_key_sign_stale'
            for field in ('attempt_id', 'node_id'):
                if expected.get(field) and data.get(field) != expected[field]:
                    return 'ssh_browser_key_sign_stale'
...
def request_browser_ssh_signature(bridge, signer_sid, browser_key, challenge, algorithm):
    ...
    if (
        ...
        or bridge._connection_cancelled.is_set()
    ):
...
def build_external_agent_wait_output_payload(bridge, state, command):
    ...
    if error_code:
        return None, error_code
    gap_detected = bool(tail['gap']['detected'])
    event_count = len(tail['events'])
    timed_out = wait_ms > 0 and event_count == 0 and not gap_detected
    status = 'gap' if gap_detected else 'changed' if event_count else 'timeout' if timed_out else 'idle'
    payload = {
        'condition': 'output',
        'status': status,
        'timed_out': timed_out,
        'changed': event_count > 0 or gap_detected,
        'wait_ms': wait_ms,
        'since_output_seq': tail['since_output_seq'],
        'output_seq': tail['output_seq'],
    ...
    if external_agent_flag_enabled(command.get('include_events')):
        formatted_tail = format_external_agent_tail_payload(
            tail,
            strip_ansi=should_external_agent_strip_ansi(command),
        )
        payload['events'] = formatted_tail['events']
        payload['limit'] = formatted_tail['limit']
        if formatted_tail.get('strip_ansi'):
            payload['strip_ansi'] = True
            payload['data_format'] = formatted_tail['data_format']
    return payload, None

def build_external_agent_wait_quiet_payload(bridge, state, command):
    quiet_ms = parse_external_agent_screen_quiet_ms(command.get('quiet_ms'))
    if quiet_ms <= 0:
        return None, AGENT_ERROR_ACTION_INVALID_DATA
    screen_wait, error_code = build_external_agent_screen_wait_payload(
        bridge,
        state,
    ...
    return {
        'condition': 'quiet',
        'status': 'timeout' if screen_wait['timed_out'] else 'settled',
        'timed_out': screen_wait['timed_out'],
        'settled': screen_wait['settled'],
        'wait_ms': screen_wait['wait_ms'],
        'quiet_ms': screen_wait['quiet_ms'],
        'terminal_quiet_ms': screen_wait.get('terminal_quiet_ms'),
        'output_seq': screen_wait.get('output_seq'),
    }, None

def build_external_agent_wait_payload(bridge, state, command):
    condition = parse_external_agent_wait_condition(command)
    if condition == 'output':
        return build_external_agent_wait_output_payload(bridge, state, command)
    if condition == 'quiet':
...
def external_agent_build_terminal_input_action(state, data, submit_after=False, input_metadata=None):
    action = {
        'action_type': AGENT_ACTION_TERMINAL_INPUT,
        'terminal_id': state.terminal_id,
        'data': data,
...
def build_agent_action(state, proposal, requires_approval):
    ...
    input_kind = proposal.get('input_kind') if isinstance(proposal.get('input_kind'), str) else None
    if input_kind in EXTERNAL_AGENT_SEND_INPUT_KINDS:
        action['input_kind'] = input_kind
    key_names = proposal.get('key_names')
    if isinstance(key_names, list) and all(isinstance(key, str) for key in key_names):
        action['key_names'] = list(key_names)
        action['key_count'] = len(key_names)
...
def public_agent_action(action):
    payload = {
        ...
        'key_names': list(action.get('key_names') or []),
        'key_count': action.get('key_count'),
...
def emit_agent_action_result(sid, action, status, error_code=None):
    ...
    payload['status'] = AGENT_STATUS_FAILED if error_code else status
    if error_code:
...
def emit_agent_action_failure(sid, action, error_code):
...
def find_agent_action_for_decision(state, action_id, proposal_id):
...
def begin_terminal_start(session_token, terminal_id, sid=None, attempt_id=None):
...
def finish_terminal_start(session_token, terminal_id, start_token):
    with terminal_start_lock:
        ...
        pending_terminal_bridges.pop(key, None)
        pending_terminal_start_context.pop(key, None)
        return True

def cancel_terminal_starts(session_token, terminal_id=None, *, sid=None, attempt_id=None):
    pending = []
    with terminal_start_lock:
        for key in list(pending_terminal_starts):
            context = pending_terminal_start_context.get(key, {})
            if key[0] != session_token or (terminal_id is not None and key[1] != terminal_id):
                continue
            if sid is not None and context.get('sid') != sid:
                continue
...
def emit_connection_error(sid, message, error_code=None, action_type=None, action_message=None,
                          action_question=None, action_id=None, terminal_id=TERMINAL_ID_MAIN, attempt_id=None, route_context=None):
    socketio.emit(
        'ssh_output',
        {
...
def build_external_agent_discovery_payload(
    ...
    return {
        ...
        'operations': {
            ...
            'send': {'op': 'send', 'kind': 'text', 'text': 'pwd\n'},
            'send_keys': {'op': 'send', 'kind': 'keys', 'keys': ['Down', 'Enter']},
            'send_submit': {'op': 'send', 'kind': 'text', 'text': 'codex prompt', 'submit_after': True},
            'send_capture': {
                'op': 'send',
                'kind': 'text',
                'text': 'pwd\n',
...
def build_external_agentinfo_status_hints(states=None):
    ...
    with agent_lock:
        ...
        visible_states = [
            state for state in bridged_states
            if is_external_agent_state_visible(state)
        ]
        return {
            'agent_attached': bool(states),
            'terminal_count': len(bridged_states),
            'external_agent_available': bool(visible_states),
            'modes': sorted({state.mode for state in states}),
            'paused': any(state.paused or state.mode == AGENT_MODE_PAUSED for state in states),
            'human_input_lease_active': any(
                is_agent_human_input_lease_active(state, now=now) for state in states
            ),
            'privacy_blocking': any(
                state.privacy_state in AGENT_CONTEXT_BLOCKING_PRIVACY_STATES for state in states
            ),
        }
...
def external_agentinfo():
    ...
    payload = build_external_agentinfo_payload(
        base_url=request.host_url.rstrip('/'),
        agentinfo_path=EXTERNAL_AGENT_INFO_PATH,
    )
    return external_agent_json_response(payload)
...
def on_agent_detach(data):
    ...
    for sid, action in cancelled:
        emit_agent_action_failure(sid, action, AGENT_REASON_DETACHED)
    socketio.emit(
        AGENT_EVENT_STATE,
        {
            'session_id': get_agent_session_id(session_token),
...
def start_terminal_backend(sid, session_token, payload, start_token):
    ...
    try:
        ...
        with terminal_start_lock:
            if not is_current_terminal_start(session_token, terminal_id, start_token):
                close_bridge(bridge)
                return
            pending_terminal_bridges[(session_token, terminal_id)] = bridge
    ...
    if success:
        with terminal_start_lock:
            current = is_current_terminal_start(session_token, terminal_id, start_token)
            authorized = socket_session_tokens.get(sid) == session_token
            if current:
                finish_terminal_start(session_token, terminal_id, start_token)
            if current and authorized:
                previous = pop_bridge(session_token, terminal_id)
                ...
                previous = None
        close_bridge(previous)
        if not current or not authorized:
            close_bridge(bridge)
            return
        connected_payload = {'message_type': 'ssh_connected'}
        if payload.get('attempt_id'):
            connected_payload['attempt_id'] = payload['attempt_id']
    ...
    with terminal_start_lock:
        if (not is_current_terminal_start(session_token, terminal_id, start_token)
                or socket_session_tokens.get(sid) != session_token):
            finish_terminal_start(session_token, terminal_id, start_token)
            failure = None
        else:
            ...
            finish_terminal_start(session_token, terminal_id, start_token)
            emit_connection_error(
                sid, failure['message'], error_code=failure.get('error_code'),
                action_type=failure.get('action_type'), action_message=failure.get('action_message'),
                action_question=failure.get('action_question'), action_id=failure.get('action_id'),
                terminal_id=terminal_id, attempt_id=payload.get('attempt_id'),
                route_context=result.get('route_context') if isinstance(result, dict) else None)
    close_bridge(bridge)
...
def on_start_ssh(data):
    ...
    start_token = begin_terminal_start(session_token, payload['terminal_id'], request.sid, payload.get('attempt_id'))
    if payload.get('browser_key') or payload.get('route'):
...
def on_ssh_host_key_action(data):
    ...
    if not is_ssh_allowed_for_client(client_ip, browser_authorized=browser_authorized):
        ...
        payload, error = validate_start_ssh_payload({
            ...
            'host': data.get('host'),
            'port': data.get('port'),
            'host_key_alias': data.get('host_key_alias'),
    ...
    socketio.emit('ssh_output', {
        ...
        'action_id': data.get('action_id'), 'operation': data.get('operation'),
        'action_type': action.action_type if action else None,
        'attempt_id': action.metadata.get('attempt_id') if action else None,
    }, room=request.sid)

@socketio.on('cancel_ssh_start')
def on_cancel_ssh_start(data):
    session_token = socket_session_tokens.get(request.sid)
    terminal_id = validate_terminal_id_payload(data)
    if not session_token or not terminal_id:
        return
...
def on_close_terminal(data):
    ...
    if not session_token or not terminal_id:
        return
    cancel_terminal_starts(session_token, terminal_id, sid=request.sid)
    browser_ssh_sign_request_store.discard(session_token, terminal_id=terminal_id, sid=request.sid)
    bridge = get_allowed_bridge(session_token, terminal_id, request.sid, emit_error=True)
    if bridge:
        bridge.emit_output({
